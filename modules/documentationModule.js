            ...
            readData.readFile( chatDataFileName,
                ...
                function ( err, data ) {
                    ...
                    theCommands.forEach( ( key, index ) => {
                        ...
                        writeData.writeFileSync( chatDocOutputFile, "<tr>\n" + "<td class=\"tg-0lax\">" + key + "</td>", { flag: 'a+' } );

                        // write the messages in an unordered list
                        writeData.writeFileSync( chatDocOutputFile, "<td class=\"tg-0lax\"><ul>", { flag: 'a+' } );
                        theMessages.forEach( ( key, index ) => {
                            // console.log( key );
                            writeData.writeFileSync( chatDocOutputFile, "<li>" + key, { flag: 'a+' } );
                        } );
                        writeData.writeFileSync( chatDocOutputFile, "</ul></td>", { flag: 'a+' } );

                        // write the pictures as 100 wide images in an unordered list
                        // console.log( thePictures );
                        writeData.writeFileSync( chatDocOutputFile, "<td class=\"tg-0lax\">", { flag: 'a+' } );
                        if ( thePictures !== undefined ) {
                            thePictures.forEach( ( key, index ) => {
                                // console.log( key );
                                writeData.writeFileSync( chatDocOutputFile, "<img src=\"" + key + "\" width=\"100\"> ", { flag: 'a+' } );
                            } );
                        }
                        writeData.writeFileSync( chatDocOutputFile, "</td>", { flag: 'a+' } );

                        writeData.writeFileSync( chatDocOutputFile, "</tr>", { flag: 'a+' } );
                    } );

                    writeData.writeFileSync( chatDocOutputFile, "</tbody>\n" +
                        "</table>\n" +
